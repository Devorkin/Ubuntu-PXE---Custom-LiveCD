...
function prepare_custom_livecd() {
    livecd_dist_name=$2
    short_description=$1
...
#     apt install -y conky curl exfat-fuse exfat-utils glances ipmitool lldpd mdadm net-tools nfs-common openssh-server screen smartmontools traceroute vim
...
#     ### Temporary code ###
#     wget https://github.com/Devorkin/Ubuntu-PXE---Custom-LiveCD/blob/main/automatik.zip?raw=true -O /tmp/automatik.zip
#     unzip /tmp/automatik.zip -d /tmp
#     mkdir -p /usr/local/lib/conky
...
#     umount /dev
    ...
    sed -i "s|#define DISKNAME  $original_disk_name|#define DISKNAME  Ubuntu 20.04 -- ${short_description}|" $livecd_extract_path/README.diskdefines
...
LABEL $line_num
  MENU LABEL Ubuntu -- 20.04 - ${short_description}, from NFS
...
prepare_custom_livecd "Devorkin.net edition" "ubuntu-20.04-bare_metal-custom"
virtual_machine='True'
prepare_custom_livecd "Devorkin.net VM-edition" "ubuntu-20.04-vm-custom"